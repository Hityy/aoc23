...
const indexOfMany = (text: string[], searchString: string, position = 0, results = []) => {
...
const matchDigits = (text: string[], digits: string[]) =>
    digits.reduce((acc, digit) => acc.concat(indexOfMany(text, digit)), []);
...
pipe(
    ...
    parseData,
    map(split('')),
    map(s => matchDigits(s, numbers)
        .sort((a, b) => a.index - b.index)
        .map(s => valuesDict[s.searchString] || s.searchString)),
    map(s => s.at(0) + s.at(-1)),
    sum,
    log
);
...
pipe(
    ...
    parseData,
    map(split('')),
    map(s => matchDigits(s, digitsAndNumbers)
        .sort((a, b) => a.index - b.index)
        .map(s => valuesDict[s.searchString] || s.searchString)),
    map(s => s.at(0) + s.at(-1)),
    sum,
